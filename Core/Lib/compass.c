...
static char file_cali_name[] = "cali_value.txt";
...
const int16_t max_change = 700;
uint16_t wrong_val_count;
...
void read_calibrate_file(){
   int status = black_box_read(&calib_file,file_cali_name,buffer_,BUFFER_LENGTH);
   if(!status){
...
static void compass_calibrate(){
  ...
  // create file
	...
	while(TRUE){
        ...
        deltaChange_[X] = as.x -  last_axis[X];
        deltaChange_[Y] = as.y -  last_axis[Y];
        deltaChange_[Z] = as.z -  last_axis[Z];
        // ignore wrong value and set to zero
        if(abs(deltaChange_[X]) >  max_change || abs(deltaChange_[Y]) >  max_change || abs(deltaChange_[Z]) >  max_change){
          as.x = 0;
          as.y = 0;
          as.z = 0;
          wrong_val_count ++;
    ...
    // check all min value is negative sig
    if(min_val[X] > 0 || min_val[Y] > 0 || min_val[Z] > 0){
          // error
          // do sth
    }
    // check all max value is positive sig
